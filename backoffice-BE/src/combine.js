...
app.get("/class_profile/:class_uid", async (req, res) => {
  try {
    ...
    // Get class schedule
    const [schedule] = await pool.query("SELECT * FROM class_schedule WHERE class_id = ?", [
      classData.id,
    ]);
...
app.patch("/admin/users/:email/status", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET subscription_status = ? WHERE email = ?", [status, email]);
...
app.delete("/admin/users/:email", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET subscription_status = ? WHERE email = ?", [
      "deleted",
      email,
    ]);
...
app.delete("/admin/classes/:class_uid", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE classes SET status = ? WHERE class_uid = ?", ["deleted", class_uid]);
...
app.delete("/admin/classes/:class_uid/unenroll/:student_email", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM students WHERE email = ? AND class_id = ?", [
      student_email,
      class_uid,
    ]);
