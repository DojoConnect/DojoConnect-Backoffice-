...
export class ClassEnrollmentRepository {
  static create = async (data: INewClassEnrollment, tx: Transaction) => {
  ...
  static fetchEnrollmentsByClassId = async (
    ...
    return await tx.select().from(classEnrollments).where(eq(classEnrollments.classId, classId));
  };
  ...
  static fetchActiveEnrollmentsByStudentIds = async (
    ...
    return await tx
      ...
      .from(classEnrollments)
      .where(
        and(inArray(classEnrollments.studentId, studentIds), eq(classEnrollments.active, true)),
      );
  };
  ...
  static fetchActiveEnrollmentsByClassIds = async (
    ...
    return await tx
      .select()
      .from(classEnrollments)
      .where(and(inArray(classEnrollments.classId, classIds), eq(classEnrollments.active, true)));
  };
}
