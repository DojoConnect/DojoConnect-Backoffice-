...
export class StudentService {
  ...
  static fetchAllInstructorStudents = async (
    instructorId: string,
    txInstance?: Transaction,
    ...
    const execute = async (tx: Transaction) => {
      ...
      const enrollments = await ClassEnrollmentRepository.fetchActiveEnrollmentsByClassIds(
        classIds,
        tx,
      );
      ...
      const studentDetails = await StudentRepository.fetchStudentsWithUsersByIds(
        uniqueStudentIds,
        tx,
      );

      return studentDetails.map(
        (row) =>
  ...
  static fetchAllDojoStudents = async (
    dojoId: string,
    txInstance?: Transaction,
  ): Promise<StudentWihUserDTO[]> => {
    const execute = async (tx: Transaction) => {
