...
describe("Parent Service", () => {
    ...
    beforeEach(() => {
        ...
        sendChildAddedEmailToParentSpy = vi.spyOn(MailerService, "sendChildAddedEmailToParent").mockResolvedValue();
        sendChildWelcomeEmailSpy = vi.spyOn(MailerService, "sendChildWelcomeEmail").mockResolvedValue();
        sendChildAddedNotificationSpy = vi.spyOn(NotificationService, "sendChildAddedNotification").mockResolvedValue();
        sendWelcomeNotificationToChildSpy = vi.spyOn(NotificationService, "sendWelcomeNotificationToChild").mockResolvedValue();
    ...
    describe("getClassesEnrolledByChildren", () => {
        ...
        it("should return enrolled classes", async () => {
            ...
            expect(fetchActiveEnrollmentsByStudentIdsSpy).toHaveBeenCalledWith(["student-1"], expect.anything());
        ...
        it("should handle no instructor", async () => {
         ...
         it("should throw NotFoundException if parent not found", async () => {
            getOneParentByUserIdSpy.mockResolvedValue(null);
            await expect(ParentService.getClassesEnrolledByChildren({ currentUser })).rejects.toThrow(NotFoundException);
         });
    });
});
