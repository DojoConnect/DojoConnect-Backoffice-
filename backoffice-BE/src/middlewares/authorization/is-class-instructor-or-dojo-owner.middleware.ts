...
export const isClassInstructorOrDojoOwnerMiddleware = async (
  ...
  try {
    ...
    if (req.user.role === Role.Instructor) {
      ...
      if (dojoClass.instructorId !== dojoInstructor.id) {
        throw new ForbiddenException(
          "Access Denied: You are not the assigned instructor for this class",
        );
      }
    ...
    throw new ForbiddenException(
      "Access Denied: You are not the owner of this Dojo or the assigned instructor for this class",
    );
  } catch (error) {
